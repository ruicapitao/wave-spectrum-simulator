...
- **User-selected spectrum**: a dropdown model option — load your own S(f) data via text input (f, S pairs), file upload (.csv/.txt), or example button; includes **agreement analysis** (Pearson r, RMSE, KS statistic vs all 5 models) and overlay comparison when data is loaded
...
- **Time-domain realization**: generate wave elevation time series via random-phase summation (up to 3600 s), with configurable time step (0.01–1.0 s), CSV download, and FFT spectrum overlay
...
- **CSV export** of current spectrum data, all visible overlay models, and FFT-derived spectrum in one file
