...
- **Multiple spectrum overlay**: check any combination of models (distinct colors/dashes) for side-by-side comparison; User-selected overlay (pink) appears when data is available
...
- **Engineering report** with spectral moments, derived parameters, model-specific commentary, and custom-spectrum agreement ranking — exportable to clipboard or HTML
...
- **Main spectrum plot**: Spectral density S(f) vs frequency. The peak is marked with a star. Filled area shows total energy. Additional overlays appear for Ochi-Hubble (swell/sea components), TMA (deep-water equivalent), checked comparison models, and FFT-derived spectrum.
...
- **Generate Report**: Produces a structured document with input parameters, spectral moments, and custom-spectrum agreement ranking.
